...
Section Perm_ind.
  ...
  Proof.
    ...
    - apply : Htrans (Hcons a _ _ (IHn _ Hs' _ (perm_to_rem Hb))) _.
      ...
      + rewrite -(perm_cons b). apply : perm_trans Hperm.
        ...
        * apply /permP => g /=. by rewrite addnCA.
        * rewrite perm_cons perm_sym. exact : perm_to_rem.
  Qed.
  ...
  Proof.
    move => Hcons Hcons2. by apply : perm_ind =>[|u s t->->||a b s t<-].
  Qed.
  ...
  Lemma perm_imply_ind (P Q:seq T -> seq T -> Prop) :
    ...
    (forall a b s t,
        P [:: b, a & s] t -> Q [:: a, b & s] t -> Q [:: b, a & s] t) ->
    forall s t, perm_eq s t -> P s t -> Q s t.
  Proof.
    move => /perm_Wl HPl /perm_Wr HPr HPcons Hnil Htrans Hcons Hcons2 s.
    have [n] := ubnP (size s).
    elim : n s =>[|n IHn][|a s]//=;
    ...
    - move : (HPl _ _ Hperm HPas) (HPr _ _ Hperm HPas) (perm_to_rem Hb)
      => HpPl HpPr Hsb.
      have Hbas : perm_eq [:: b, a & rem b s] (b :: t)
        by apply : perm_trans Hperm;
        rewrite -perm_rcons /= perm_cons perm_rcons perm_sym.
      have HPasr : P (a :: s) (a :: b :: rem b s)
        by apply /HpPr; rewrite perm_cons.
      apply : Htrans
                (Hcons _ _ _ HPasr (IHn _ Hs _ Hsb (HPcons _ _ _ HPasr))) _.
      apply /(Hcons2 _ _ _ _ (HpPl _ (perm_trans _ Hperm)));
        [by rewrite perm_cons perm_sym|apply /Hcons; [|apply /IHn=>/=]].
      + exact : HpPl.
      + rewrite (size_rem Hb).
          by case : s Hb Hs {Hperm HPas HpPr Hsb Hbas HPasr}.
...
(* example *)
  ...
  Proof.
    ...
    =>[s t /(perm_map f)/perm_uniq->|a s /andP[]|
       |u s t Hsu Hut y|a s t _ H y|a b s t]//=.
    - by rewrite Hsu Hut.
    - by rewrite H.
